export function createMapOverlay({
  ...
  const visitedBuildings = new Set();
  ...
  function renderMap() {
    ...
    for (const buildingData of buildings) {
      ...
      const width = (buildingData.width || building.userData.width || 20) * mapScale;
      const depth = (buildingData.depth || building.userData.depth || 20) * mapScale;
      ...
      ctx.strokeRect(pos.x - width / 2, pos.y - depth / 2, width, depth);
    ...
    document.getElementById('map-chunks-loaded').textContent = `Chunks: ${activeChunks.size}`;
    document.getElementById('map-buildings-discovered').textContent = `Buildings: ${discoveredCount}/${totalCount}`;
  }

  function updateVisitedBuildings() {
    const playerPos = getPlayerPosition();
    const VISIT_RADIUS = 30;
    const VISIT_RADIUS_SQ = VISIT_RADIUS * VISIT_RADIUS;
    ...
    for (const buildingData of buildings) {
      ...
      if (distSq < VISIT_RADIUS_SQ) {
        visitedBuildings.add(buildingId);
        console.log(`🗺️ Discovered building #${buildingId}`);
      }
    }
  }
  ...
  function initialize() {
    ...
    console.log('✓ Map canvas initialized:', mapCanvas.width, 'x', mapCanvas.height);
  }
    ...
    if (mapCanvas) {
      mapCanvas.addEventListener('mousemove', (e) => {
        ...
        for (const buildingData of buildings) {
          ...
          const buildingSize = {
            w: (buildingData.width || building.userData.width || 20) * mapScale,
            h: (buildingData.depth || building.userData.depth || 20) * mapScale
          };

          if (mouseX >= buildingScreenPos.x - buildingSize.w / 2 &&
        ...
        if (hoverInfo) {
          if (hoveredBuilding) {
            const hoveredData = buildings.find(b => b.mesh === hoveredBuilding);
            ...
            const buildingType = (hoveredData && hoveredData.isSpecialBuilding) || hoveredBuilding.userData.isSpecialBuilding ?
              (hoveredBuilding.userData.specialType || 'Special') : 'Building';
