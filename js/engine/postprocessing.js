...
export function createPostProcessing(options) {
  const {
    THREE,
    renderer,
    scene,
  ...
  MotionBlurShader.uniforms.delta.value = new THREE.Vector2(0, 0);
  ...
  const filmPass = new FilmPass(filmIntensity, 0.025, 648, filmGrayscale);
  filmPass.enabled = filmEnabled;
