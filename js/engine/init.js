...
let postprocessingController = null;
let postprocessingPromise = null;
let mapOverlay = null;
let mapOverlayPromise = null;
let interiors = null;
let interiorsPromise = null;
let rocketAssets = null;
let rocketAssetsPromise = null;

async function loadPostProcessing() {
  if (!postprocessingPromise) {
    postprocessingPromise = import('./postprocessing.js').then((module) => module);
  }
  return postprocessingPromise;
}

async function loadMapOverlay() {
  if (!mapOverlayPromise) {
    mapOverlayPromise = import('../map/overlay.js').then((module) => module);
  }
...
async function loadInteriors() {
  if (!interiorsPromise) {
    interiorsPromise = import('../interiors/loader.js').then((module) => module);
  }
  return interiorsPromise;
}

async function loadRocketAssets() {
  if (!rocketAssetsPromise) {
    rocketAssetsPromise = import('./rocket-assets.js').then((module) => module);
  }
  return rocketAssetsPromise;
}

let INTERIORS_GRID_SIZE = 1;
let INTERIORS_WALL_HEIGHT = 2.5;
let INTERIORS_WALL_THICKNESS = 0.1;
let INTERIORS_ASSETS = {};
let INTERIORS_ROOM_CONFIGS = {};
let furniturePropsPool = {};
let miscPropsPool = [];

let initInteriors = null;
let toggleInteriorsMode = null;
let clearInteriorRoom = () => {};
let onInteriorsClick = () => {};
let toggleAssetMenu = () => {};
let isAssetMenuOpen = () => false;
let setupRandomizerButton = () => {};

async function ensureInteriorsLoaded() {
  if (!interiors) {
    const module = await loadInteriors();
    interiors = module;
    ({
      INTERIORS_GRID_SIZE,
    ...
    initInteriors = module.initInteriors;
    toggleInteriorsMode = module.toggleInteriorsMode;
    clearInteriorRoom = module.clearInteriorRoom;
    onInteriorsClick = module.onInteriorsClick;
    toggleAssetMenu = module.toggleAssetMenu;
    isAssetMenuOpen = module.isAssetMenuOpen;
    setupRandomizerButton = module.setupRandomizerButton;
    interiorRoomObjects = module.interiorRoomObjects;
    interiorInteractiveObjects = module.interiorInteractiveObjects;
    interactiveInteriorsObjects = module.interactiveInteriorsObjects;
  }
  return interiors;
}

let interiorsInitialized = false;

async function ensureInteriorsInitialized() {
  const module = await ensureInteriorsLoaded();
  if (!interiorsInitialized) {
    module.initInteriors({
      THREE,
      scene,
      worldRoot,
      playerBody,
    ...
    interiorsGroup = module.interiorsGroup;
    interiorsMode = module.interiorsMode;
    interiorsInitialized = true;
  }
  return module;
}
...
async function ensurePostProcessing() {
  if (!postprocessingController) {
    const module = await loadPostProcessing();
    postprocessingController = module.createPostProcessing({
      THREE,
      renderer,
      scene,
      camera,
...
async function ensureMapOverlay() {
  if (!mapOverlay) {
    const module = await loadMapOverlay();
    mapOverlay = module.createMapOverlay({
      document,
      getPlayerPosition: () => {
    ...
    mapOverlay.setupControls();
  }
  return mapOverlay;
}

async function ensureRocketAssetsLoaded() {
  if (!rocketAssets) {
    rocketAssets = await loadRocketAssets();
  }
  return rocketAssets;
}

async function ensureRocketEntities() {
  const module = await ensureRocketAssetsLoaded();
  if (!rocketMesh || !rocketBody) {
    const created = module.createRocket({
      THREE,
      BufferGeometryUtils,
      scene,
      physics,
    ...
    rocketMesh = created.rocketMesh;
    rocketBody = created.rocketBody;
  }
  if (!atmosphereLayer) {
    atmosphereLayer = module.createAtmosphereLayer({
      THREE,
      scene,
  ...
  if (!starField) {
    starField = module.createStarField({ THREE, scene });
  }
  if (!giantFinger) {
    giantFinger = module.createGiantFinger({ THREE, scene });
  }
}
...
// Cached scene object lists (avoid full scene traversal each frame)
const cachedObjects = {
  grass: new Set(),
  spinners: new Set(),
  lodNodes: new Set(),
...
const chunkObjectCache = new Map(); // Map<"x,z", {grass, spinners, lodNodes, windowMeshes, sillMeshes}>
...
let interactiveInteriorsObjects = [];
 ...
 // 
 // 1. ✅ INSTANCING (Reduce Draw Calls):
 ...
 //
 // 2. ✅ THREE.LOD FOR BUILDINGS (Reduce GPU Triangle Count):
 ...
 //
 // 3. ✅ GEOMETRY MERGING (Reduce Draw Calls):
 ...
 //
 // 4. ✅ SHADOW OPTIMIZATION (Reduce GPU Load):
 ...
 //
 // 5. ✅ DISTANCE-BASED MATERIAL SWAPPING (Reduce Shader Complexity):
 //    - Beyond 150m: Swap MeshStandardMaterial → MeshBasicMaterial
...
function createDistantSkyline() {
  ...
  console.log(`🏙️ Creating dense skyline: ${buildingCount} buildings (density: ${optimizationConfig.skylineDensity}x)`);
  ...
  console.log(`✓ Skyline created with ${skylineGroup.children.length} buildings`);
  return skylineGroup;
}
...
function createPlanetoidSkyline() {
  ...
  console.log(`🌍 Adding skyline to planetoid: ${buildingCount} buildings (altitude: ${altitude.toFixed(0)}m)`);
  ...
  console.log(`✓ Added ${buildingCount} skyline buildings to planetoid`);
}
...
function initializeInstancedManagers() {
  ...
  console.log('✓ Instanced mesh managers initialized (trees, lights, windows, cars, sills, decorations, pedestrians, doors, foundations)');
}
...
let lastMapUpdate = 0;
let lastWindowSillUpdate = 0;
const DROPPED_VIEWPORT_FPS = 8;
const DROPPED_VIEWPORT_READBACK_INTERVAL = 1000 / DROPPED_VIEWPORT_FPS;
let lastDroppedViewportReadback = 0;
let droppedViewportPixels = null;
let droppedViewportImageData = null;
let droppedViewportTempCanvas = null;
let droppedViewportTempCtx = null;
let droppedViewportHudCanvas = null;
let droppedViewportHudCtx = null;
...
let jarShakePhase = 0; // Phase for shake animation (0-2π)
...
function createSplatter(color, angle, height) {
  ...
  console.log(`💥 Creating wrapped pixelated splatter: ${splatterSize.toFixed(3)}m size with texture`);
...
function createDialogueCard(text) {
  ...
  console.log(`🗣️ Material created - transparent: ${material.transparent}, opacity: ${material.opacity}, depthTest: ${material.depthTest}`);
  ...
  console.log(`🗣️ Created dialogue card mesh with text: "${text}", visible: ${card.visible}, renderOrder: ${card.renderOrder}, lines: ${lines.length}`);
...
function updatePedestrianDialogue(pedMesh, state, jarObj = null) {
  console.log(`🗣️ updatePedestrianDialogue called - state: ${state}, pedMesh:`, pedMesh);
  ...
  if (existingCard) {
    ...
    if (existingCard.userData.dialogueState === state) {
      ...
      if (age < 3000) {
        console.log(`🗣️ Keeping existing card (age: ${age}ms)`);
        return; // Keep for 3 seconds
      }
    }
    console.log(`🗣️ Removing old dialogue card`);
  ...
  console.log(`🗣️ Should speak? ${shouldSpeak} (30% chance)`);
  ...
  if (state === 'jarClosed' && jarObj) {
    ...
    jarObj.pleaCount++;
    console.log(`🗣️ Plea count for this pedestrian: ${jarObj.pleaCount}`);
    ...
    console.log(`🗣️ Dialogue stage: ${actualState}`);
  }
  
  // Select random phrase from appropriate state
  const phrases = DIALOGUE_PHRASES[actualState];
  if (!phrases || phrases.length === 0) {
    console.log(`🗣️ No phrases found for state: ${actualState}`);
    return;
  }
  ...
  if (jarObj && pluckJar) {
    ...
    console.log(`🗣️ Pedestrian turning to face camera (angle: ${targetAngle.toFixed(2)})`);
  }
  
  console.log(`🗣️ Creating dialogue card: "${finalPhrase}" (${isPlural ? 'plural' : 'singular'}, ${livingPedCount} pedestrian(s))`);
  ...
  console.log(`🗣️ Dialogue card added to pedMesh!`, {
...
function updateDialogueCards() {
  ...
  if (draggingObject) {
    ...
    if (shouldDebug) console.log(`🗣️ Dragging object detected. Is pedestrian? ${!!isPedestrian}`);
    if (isPedestrian) {
  ...
  if (shouldDebug) {
    console.log(`🗣️ Checking jar objects - count: ${jarObjects.length}, jarState: ${jarState}`);
  ...
  jarObjects.forEach((jarObj, index) => {
    if (jarObj.isPedestrian && jarObj.mesh && !jarObj.isBurst) {
      ...
      if (shouldDebug) console.log(`🗣️ Jar object ${index} - pedestrian in jar, state: ${state}`);
  ...
  scene.traverse(obj => {
    if (obj.userData.isDialogueCard && obj.parent) {
      ...
      if (cardCount === 1 && shouldDebug) {
        console.log(`🗣️ Found dialogue card:`, {
      ...
      if (!obj.visible) {
        obj.visible = true;
        if (shouldDebug) console.log(`🗣️ Forcibly showing hidden dialogue card`);
      }
      ...
      if (age > maxAge) {
        console.log(`🗣️ Removing old dialogue card (age: ${age}ms)`);
        removedCount++;
        if (obj.parent) {
  ...
  if (shouldDebug && (cardCount > 0 || removedCount > 0)) {
    console.log(`🗣️ Dialogue cards found: ${cardCount}, removed: ${removedCount}`);
  }
}
...
function showJarPeopleReactions(burstJarObj) {
  ...
  livingPedestrians.forEach(jarObj => {
    if (Math.random() < reactionChance) {
      ...
      console.log(`🗣️ Jar person reacting to splatter: "${randomReaction}"`);
  ...
  console.log(`💥 ${Math.floor(livingPedestrians.length * reactionChance)} jar people reacted to the splatter!`);
}
...
function shakeJar() {
  ...
  jarObjects.forEach((jarObj, index) => {
    ...
    if (jarObj.isPedestrian && Math.random() < BURST_CHANCE) {
      ...
      console.log('💥 Pedestrian marked for burst!');
    }
  });
}
...
function updateJarPhysics(deltaTime) {
  ...
  if (jarShaking) {
    ...
    if (shakeProgress < 1.0) {
      ...
      jarShakeOffsetX = 0;
      jarShakePhase = 0;
      console.log('🫙 Jar shake complete');
    }
  } else {
    jarBaseOffsetY = 0;
    jarShakeOffsetX = 0;
  ...
  for (let i = jarObjects.length - 1; i >= 0; i--) {
    ...
    if (jarObj.shouldBurst && !jarObj.hasBurst && now >= jarObj.burstTime) {
      ...
      console.log('💥 Pedestrian burst! Body now horizontal, splatters added to jar. Rotation:', {
...
async function init() {
  ...
  // Post-processing (lazy-loaded on demand)
  composer = null;
  ...
  console.log('✓ Distant skyline created');
  ...
  console.log(`🏙️ Loaded ${activeChunks.size} initial chunks in ${Math.ceil(initialRadius * 2 + 1)}x${Math.ceil(initialRadius * 2 + 1)} grid`);
  ...
  // Create bike and character Y (rocket assets are lazy-loaded)
  createBike();
  createCharacterY();
  console.log('Bike and character Y created');
  ...
  // Interiors system is lazy-loaded on demand
...
function updateChunks() {
  ...
  if (newChunkKeys.length > 0) {
    console.log(`📋 Queued ${newChunkKeys.length} new chunks for loading (queue size: ${chunkLoadQueue.length}, active: ${activeChunks.size})`);
  }
...
function registerCachedObject(cacheKey, obj, cacheEntry) {
  if (!obj || !cachedObjects[cacheKey]) return;
  cachedObjects[cacheKey].add(obj);
  if (cacheEntry && cacheEntry[cacheKey]) {
    cacheEntry[cacheKey].push(obj);
  }
}

function cacheChunkObjects(key, chunkRoot) {
  if (!chunkRoot) return;
  if (chunkObjectCache.has(key)) {
    clearChunkCache(key);
  }
  const cacheEntry = {
    grass: [],
  ...
  chunkRoot.traverse(obj => {
    if (obj.userData?.isGrass) {
      registerCachedObject('grass', obj, cacheEntry);
    }
    if (obj.userData?.isSpinner) {
      registerCachedObject('spinners', obj, cacheEntry);
    }
    if (obj.isLOD) {
      registerCachedObject('lodNodes', obj, cacheEntry);
    }
    if (obj.userData?.isWindow) {
      registerCachedObject('windowMeshes', obj, cacheEntry);
    }
    if (obj.userData?.isSill) {
      registerCachedObject('sillMeshes', obj, cacheEntry);
    }
  });
  
  chunkObjectCache.set(key, cacheEntry);
}

function clearChunkCache(key) {
  const cacheEntry = chunkObjectCache.get(key);
  if (!cacheEntry) return;
  cacheEntry.grass.forEach(obj => cachedObjects.grass.delete(obj));
  cacheEntry.spinners.forEach(obj => cachedObjects.spinners.delete(obj));
  cacheEntry.lodNodes.forEach(obj => cachedObjects.lodNodes.delete(obj));
  cacheEntry.windowMeshes.forEach(obj => cachedObjects.windowMeshes.delete(obj));
  cacheEntry.sillMeshes.forEach(obj => cachedObjects.sillMeshes.delete(obj));
  chunkObjectCache.delete(key);
}
...
function loadChunk(key) {
  ...
  cacheChunkObjects(key, chunkRoot);
  
  console.log(`✅ Loaded chunk ${key} (${district}) with grass base and physics`);
...
function updateDistantSkylineRadius() {
  ...
  if (Math.abs(skylineRadius - lastSkylineUpdateRadius) > 100) {
    ...
    worldRoot.add(distantSkylineGroup);
    console.log(`📍 Expanded distant skyline to ${skylineRadius.toFixed(0)}m radius`);
  }
}
...
function createExpandedDistantSkyline(skylineRadius) {
  ...
  console.log(`🏙️ Created distant skyline (${inSpaceMode ? 'SPACE MODE - simple prisms' : 'GROUND MODE - with windows'}) at ${skylineRadius.toFixed(0)}m radius`);
  return skylineGroup;
}
...
function switchToMicroLOD() {
  ...
  console.log('🚀 Switching to Micro LOD mode - replacing detailed city with simple prisms');
  ...
  console.log(`✅ Micro LOD active - replaced ${savedChunkData.size} chunks with ${microLODGroup.children.length} simple prisms (no textures)`);
}

function switchFromMicroLOD() {
  if (!microLODActive) return;
  console.log('🌍 Exiting Micro LOD mode - restoring detailed chunks');
  ...
  console.log(`✅ Queued ${savedChunkData.size} chunks for restoration`);
  savedChunkData.clear();
}
...
function generateChunkBuildings(chunkRoot, chunkX, chunkZ, districtType) {
  ...
  console.log(`📐 Generated ${cityPlan.length} lots for chunk (${chunkX},${chunkZ})`);
  ...
  if (placeSchool) {
    ...
    console.log(`🏫 Placed school in chunk (${chunkX},${chunkZ})`);
  }
  ...
  if (placePolice) {
    ...
    console.log(`🚓 Placed police station in chunk (${chunkX},${chunkZ})`);
  }
  ...
  for (const lot of cityPlan) {
    ...
    if (checkBuildingCollision(lot.x, lot.z, buildingWidth, buildingDepth, placedBuildings)) {
      console.log(`⚠️ Skipping lot at (${lot.x.toFixed(1)}, ${lot.z.toFixed(1)}) - collision detected`);
      continue;
    }
  ...
  console.log(`✅ Placed ${buildingsPlaced} buildings in chunk (${chunkX},${chunkZ}) using lot system`);
...
async function createGiantFinger() {
  await ensureRocketEntities();
}
...
function createViewportHand() {
  ...
  console.log('🖐️ Viewport hand created (4 fingers + thumb)');
  return handGroup;
}
...
async function createAtmosphereLayer() {
  await ensureRocketEntities();
}
...
function createBuildingTextureOnPlanetoid() {
  ...
  console.log('🏙️ Converting distant skyline to planetoid texture...');
  ...
  console.log(`✓ Added ${rectangleCount} building texture rectangles to planetoid`);
}
...
function addCityPatchesToPlanetoid() {
  ...
  console.log(`✓ Added ${buildingCount} building decorations to planetoid`);
}
...
function updateRocket(deltaTime) {
  ...
  if (inGravityWell) {
    ...
    if (holdingThrottle) {
      if (!rocketEnginesOn) {
        ...
        console.log('🚀 Engines ignited!');
      }
    } else {
      // Released Space in gravity well - engines off
      if (rocketEnginesOn) {
        rocketEnginesOn = false;
        console.log('⚠ Engines off - rocket falling!');
      }
    }
  } else {
    ...
    if ((keys.has('Space') || keys.has('KeyE')) && !keys.has('_rocketToggleCooldown')) {
      ...
      console.log(rocketEnginesOn ? '🚀 Engines ignited!' : '⚠ Engines off - drifting in space');
    }
  }
  ...
  if (inSpace && sunLight.castShadow) {
    ...
    console.log('⚡ OPTIMIZATION: Shadows disabled in space');
  } else if (!inSpace && !sunLight.castShadow) {
    sunLight.castShadow = true;
    renderer.shadowMap.enabled = true;
    console.log('💡 Shadows re-enabled at low altitude');
  }
    ...
    if (composer && composer.passes) {
      composer.passes.forEach(pass => {
        if (pass.constructor.name === 'UnrealBloomPass' || 
          ...
          if (pass.enabled) {
            pass.enabled = false;
            console.log(`⚡ OPTIMIZATION: Disabled ${pass.constructor.name}`);
          }
        }
      });
    }
      ...
      composer.passes.forEach(pass => {
        if (pass.constructor.name === 'UnrealBloomPass' || 
          ...
          if (!pass.enabled && pass.constructor.name !== 'MotionBlurPass') { // Keep motion blur off
            pass.enabled = true;
            console.log(`💡 Re-enabled ${pass.constructor.name}`);
          }
        }
      });
    }
  ...
  if (inSpace && scene.fog) {
    scene.fog = null;
    console.log('⚡ OPTIMIZATION: Fog disabled in space');
  } else if (!inSpace && !scene.fog) {
    scene.fog = new THREE.FogExp2(0xcccccc, 0.006);
    console.log('💡 Fog re-enabled at low altitude');
  }
  ...
  if (inSpace !== lastState.inSpace) {
    if (inSpace) {
      console.log('🚀 ENTERED SPACE - All expensive features disabled');
    } else {
      console.log('🌍 RE-ENTERED ATMOSPHERE - Features re-enabled');
    }
  ...
  if (!atmosphereLayer) void createAtmosphereLayer();
  if (!starField) void createStarField();
  if (!planetoidMesh) {
    createPlanetoid();
  }
  ...
  if (altitude > 80) {
    ...
    if (planetoidMesh.userData.lodGeometry) {
      ...
      if (altitude > 1000 && planetoidMesh.userData.currentLOD !== 'low') {
        ...
        console.log('⚡ Planetoid LOD: ULTRA-LOW (16x16)');
        ...
        console.log('⚡ Planetoid LOD: MEDIUM (32x32)');
        ...
        planetoidMesh.userData.currentLOD = 'high';
        console.log('💎 Planetoid LOD: HIGH (64x64)');
      }
      ...
      if (inSpace && planetoidMesh.material !== planetoidMesh.userData.basicMaterial) {
        ...
        console.log('⚡ Planetoid material: BASIC (no lighting)');
        ...
        console.log('💡 Planetoid material: STANDARD (with lighting)');
      }
    }
  ...
  if (giantFinger) {
    if (altitude > FINGER_SHOW_ALTITUDE) {
      ...
      if (!fingerAnimStartTime) {
        ...
        fingerAnimating = false;
        console.log('☝️ Finger animation triggered - 3 second delay started');
      }
      ...
      if (fingerAnimStartTime) {
        fingerAnimStartTime = null;
        fingerAnimating = false;
        console.log('☝️ Finger animation reset - dropped below altitude');
      }
      giantFinger.visible = false;
    }
  }
...
function createPlayground(x, z, size = 15) {
  ...
  registerCachedObject('spinners', spinnerGroup);
...
function generateInteriorFromPolicy(policy) {
  ...
  console.log(`🏢 Generating ${policy.type} interior...`);
  ...
  console.log(`✅ Generated ${policy.rooms.length} rooms for ${policy.type}`);
}
...
function updatePluckCandidates() {
  ...
  if (rocketEnabled && altitude > SPACE_ALTITUDE && planetoidMesh && planetoidMesh.visible) {
    ...
    console.log('🌍 Planetoid is pluckable from space!');
  }
  ...
  if (buildingCount > 0) {
    console.log(`🏢 ${buildingCount} building(s) fully in frame and pluckable!`);
  }
...
function startDraggingObject(obj, clientX, clientY) {
  ...
  if (building && building.body) {
    console.log(`🏢 Plucking building! Converting to dynamic physics (${building.width.toFixed(1)}m × ${building.depth.toFixed(1)}m × ${building.height.toFixed(1)}m)`);
    ...
    console.log('✓ Building is now pluckable and can be moved/dropped!');
  }
...
function stopDraggingObject() {
  ...
  if (nearbyContainers.length > 0) {
    ...
    console.log('✅ Object successfully parented to container');
  }
...
function applyRoomConstructionPolicies(config) {
  ...
  if (hasBedrooms && (!config.windows || config.windows.length === 0)) {
    console.warn('⚠️ Bedroom missing windows - adding them');
  ...
  beds.forEach(bed => {
    ...
    if (!hasRug) {
      console.log(`📐 Adding rug under bed at (${bed.x}, ${bed.z})`);
      config.furniture.push({
        type: 'rug',
        x: bed.x,
        z: bed.z,
  ...
  couches.forEach(couch => {
    ...
    if (!hasRug) {
      console.log(`📐 Adding rug under couch at (${couch.x}, ${couch.z})`);
      config.furniture.push({
        type: 'rug',
        x: couch.x,
        z: couch.z - 0.5,
  ...
  if (config.mainRoom === 'living' || config.mainRoom === 'studio') {
    ...
    if (!hasTVStand || !hasTV || !hasCouch || !hasCoffeeTable) {
      console.log('🛋️ Main room missing living room essentials - would add them (currently disabled to avoid duplicates)');
    ...
    if (!config.kitchenPartition) {
      console.warn('⚠️ Main room missing kitchen partition');
      const bounds = config.floorBounds;
  ...
  fridges.forEach(fridge => {
    counters.forEach(counter => {
      ...
      const counterFacingWest = Math.abs(counterRotation - Math.PI) < 0.1; // π rad
      const counterFacingNorth = Math.abs(counterRotation - Math.PI/2) < 0.1; // π/2 rad
      const counterFacingSouth = Math.abs(counterRotation + Math.PI/2) < 0.1; // -π/2 rad
...
// toggleInteriorsMode handled via lazy-loaded module
...
class ApartmentArchitect {
  ...
  defineApartmentBounds() {
    ...
    console.log(`🏗️ Apartment bounds: ${size.width}x${size.depth}m`);
  }
  ...
  subdivideIntoRooms() {
    ...
    console.log(`🏗️ Created ${this.rooms.length} room partitions`);
  }
  ...
  calculateDoorPositions() {
    ...
    console.log(`🚪 Calculated ${this.doors.length} door positions`);
  }
  ...
  buildWalls() {
    ...
    console.log(`🧱 Built ${this.walls.length} wall segments`);
  }
...
function generateInteriorRoom(roomType, useRandom = false) {
  ...
  console.log(`🏗️ Generating ${roomType} using Architectural Planning System...`);
  const architect = new ApartmentArchitect(roomType);
  let config = architect.generate();
  
  console.log(`✅ Generated apartment with ${config.rooms.length} rooms, ${config.walls.length} walls, ${config.furniture.length} furniture items`);
...
function setupAssetLibrary() {
  ...
  libraryDiv.innerHTML = `
    ...
    <div style="font-size: 11px; color: #8ab4ff; margin-bottom: 12px;">
      Press <kbd style="background: #2a3440; padding: 2px 6px; border-radius: 3px; font-size: 10px;">ESC</kbd> or <kbd style="background: #2a3440; padding: 2px 6px; border-radius: 3px; font-size: 10px;">B</kbd> to close • Assets spawn in front of camera
    </div>
  ...
  Object.keys(ASSET_LIBRARY).forEach((categoryKey) => {
    ...
    categoryHeader.innerHTML = `
      ...
      <span id="arrow-${categoryKey}" style="transition: transform 0.3s;">▼</span>
    `;
  ...
  window.addEventListener('keydown', async (e) => {
    if (e.code === 'KeyB' && interiorsMode && !isAssetMenuOpen()) {
      ...
      console.log(`📦 Asset Library ${assetLibraryVisible ? 'opened' : 'closed'}`);
    }
...
function spawnAsset(assetDef) {
  ...
  if (handler) {
    ...
    if (spec.type === 'plate') {
      ...
      console.log(`✨ Auto-placed fork and knife with plate`);
    }
    ...
    if (spec.type === 'bowl') {
      ...
      console.log(`✨ Auto-placed spoon with bowl`);
    }
  }
...
function setupControls() {
  ...
  window.addEventListener('keydown', async (e) => {
    ...
    if (e.code === 'Escape' && isAssetMenuOpen()) {
      toggleAssetMenu();
      return;
    }
    ...
    if (e.code === 'KeyB') {
      if (interiorsMode) {
        // In interior mode: toggle asset menu
        toggleAssetMenu();
      } else {
    ...
    if (e.code === 'KeyI') {
      const module = await ensureInteriorsInitialized();
      module.toggleInteriorsMode();
      interiorsMode = module.interiorsMode;
    }
    if (e.code === 'KeyU') {
  ...
  window.addEventListener('mousedown', (e) => {
    ...
    if (jarVisible && !lassoEnabled && !viewportEquipped && e.button === 2 && pointerLocked) {
      ...
      console.log(`🫙 Jar inspect mode: ${jarInspectMode ? 'ON' : 'OFF'}`);
      return;
    }
        ...
        if (!selected && jarVisible && jarState !== 2 && !viewportEquipped) {
          ...
          if (jarIntersects.length > 0) {
            ...
            if (clickedJarObj && clickedJarObj.mesh) {
              ...
              console.log(`🫙 Plucking ${objectType} from jar!`);
              ...
              if (clickedJarObj.isBuilding) {
                ...
                if (building && building.body) {
                  ...
                  console.log('🏢 Re-enabled building physics');
                }
              }
        ...
        if (selected) {
          ...
          if (activeFrame) {
            ...
            if (isPedestrian) {
              ...
              console.log(`🗣️ Pedestrian plucked! Triggering plucked dialogue...`);
          ...
          if (selected.object.userData.isPlanetoid && selected.object === planetoidMesh) {
            console.log('🌍 Plucking entire planetoid!');
            ...
            if (jarVisible && jarState === 1) {
              console.log('🌍🫙 Auto-placing planetoid in jar!');
  ...
  function stopDraggingObject(droppedInJar = false) {
    ...
    if (droppedInJar) {
      ...
      if (obj.userData.isPlanetoid && obj === planetoidMesh) {
        console.log('🌍✨ Collected the entire planetoid into the jar!');
      ...
      if (building && building.body) {
        ...
        console.log('🏢 Disabled building physics for jar storage');
      }
      ...
      console.log(`✅ Added ${objectType} to jar environment at floor level`);
...
function setupUI() {
  ...
  document.getElementById('ssao-intensity-slider').addEventListener('input', (e) => {
  ...
  document.getElementById('ssao-radius-slider').addEventListener('input', (e) => {
  ...
  document.getElementById('dof-aperture-slider').addEventListener('input', (e) => {
  ...
  document.getElementById('dof-maxblur-slider').addEventListener('input', (e) => {
  ...
  document.getElementById('film-intensity-slider').addEventListener('input', (e) => {
  ...
  document.getElementById('vignette-intensity-slider').addEventListener('input', (e) => {
  ...
  document.getElementById('vignette-softness-slider').addEventListener('input', (e) => {
  ...
  document.getElementById('chromatic-amount-slider').addEventListener('input', (e) => {
  ...
  // Render scale
  document.getElementById('render-scale-slider').addEventListener('input', async (e) => {
  ...
  console.log('🔧 Initializing optimization controls...');
  ...
  toggleWindows.addEventListener('click', (e) => {
    ...
    console.log(`🪟 Windows: ${optimizationConfig.windows.enabled ? 'ON' : 'OFF'}`);
  });
  ...
  toggleSills.addEventListener('click', (e) => {
    ...
    console.log(`🏛️ Sills: ${optimizationConfig.sills.enabled ? 'ON' : 'OFF'}`);
  });
  ...
  togglePedestrians.addEventListener('click', (e) => {
    ...
    console.log(`🚶 Pedestrians: ${optimizationConfig.pedestrians.enabled ? 'ON' : 'OFF'}`);
  });
  ...
  toggleGrass.addEventListener('click', (e) => {
    ...
    console.log(`🌱 Grass: ${optimizationConfig.grass.enabled ? 'ON' : 'OFF'}`);
  });
  ...
  windowsSlider.addEventListener('input', (e) => {
    ...
    document.getElementById('windows-count').textContent = optimizationConfig.windows.maxCount;
    console.log(`🪟 Windows max: ${optimizationConfig.windows.maxCount}`);
  });
  ...
  sillsSlider.addEventListener('input', (e) => {
    ...
    document.getElementById('sills-count').textContent = optimizationConfig.sills.maxCount;
    console.log(`🏛️ Sills max: ${optimizationConfig.sills.maxCount}`);
  });
  ...
  pedestriansSlider.addEventListener('input', (e) => {
    ...
    document.getElementById('pedestrians-count').textContent = optimizationConfig.pedestrians.maxCount;
    console.log(`🚶 Pedestrians max: ${optimizationConfig.pedestrians.maxCount}`);
  });
  ...
  grassSlider.addEventListener('input', (e) => {
    ...
    document.getElementById('grass-count').textContent = optimizationConfig.grass.maxCount;
    console.log(`🌱 Grass max: ${optimizationConfig.grass.maxCount}`);
  });
  ...
  skylineSlider.addEventListener('input', (e) => {
    ...
    document.getElementById('skyline-density').textContent = optimizationConfig.skylineDensity.toFixed(1) + 'x';
    console.log(`🏙️ Skyline density: ${optimizationConfig.skylineDensity.toFixed(1)}x`);
  });
  
  // Apply optimization and rebuild
  document.getElementById('btn-apply-optimization').addEventListener('click', () => {
    console.log('🔧 Applying optimization settings and rebuilding...');
    ...
    console.log('  → Rebuilding instanced managers...');
    initializeInstancedManagers();
    
    // Clear and rebuild chunks
    console.log('  → Clearing city chunks...');
    cityChunks.clear();
    activeChunks.clear();
    ...
    console.log('  → Rebuilding distant skyline...');
    ...
    if (optimizationConfig.skylineWrapPlanetoid && rocketMode) {
      console.log('  → Rebuilding planetoid skyline...');
  ...
  // ==================== MAP SYSTEM SETUP ====================
  // Map overlay events are initialized when the map module is loaded.
...
function toggleGrassBlades() {
  ...
  if (grassBladesEnabled) {
    ...
    const grassMeshes = Array.from(cachedObjects.grass);
    grassMeshes.forEach(obj => {
      obj.visible = false;
      if (obj.parent) {
        obj.parent.remove(obj);
      }
    ...
    cachedObjects.grass.clear();
    chunkObjectCache.forEach(entry => {
      entry.grass = [];
    });
  }
}

function ensureDroppedViewportBuffers() {
  if (!droppedViewportRT) return false;
  const width = droppedViewportRT.width;
  const height = droppedViewportRT.height;
  
  if (!droppedViewportPixels || droppedViewportPixels.length !== width * height * 4) {
    droppedViewportPixels = new Uint8Array(width * height * 4);
    droppedViewportImageData = new ImageData(
      new Uint8ClampedArray(droppedViewportPixels.buffer),
      width,
      height
    );
    droppedViewportTempCanvas = document.createElement('canvas');
    droppedViewportTempCanvas.width = width;
    droppedViewportTempCanvas.height = height;
    droppedViewportTempCtx = droppedViewportTempCanvas.getContext('2d');
  }
  
  if (!droppedViewportHudCanvas) {
    droppedViewportHudCanvas = document.getElementById('hud-mini-canvas');
    droppedViewportHudCtx = droppedViewportHudCanvas?.getContext('2d');
  }
  
  return Boolean(droppedViewportHudCanvas && droppedViewportHudCtx && droppedViewportTempCtx);
}

function updateStats(time) {
  if (time - lastStatsUpdate < DOM_UPDATE_INTERVAL) return;
  lastStatsUpdate = time;
  
  if (!statsElements) {
    statsElements = {
      scale: document.getElementById('stat-scale'),
      worldScale: document.getElementById('stat-world-scale'),
      position: document.getElementById('stat-position'),
      velocity: document.getElementById('stat-velocity'),
      buildings: document.getElementById('stat-buildings'),
  ...
  statsElements.scale.textContent = playerScale.toFixed(2) + 'x';
  statsElements.worldScale.textContent = worldScale.toFixed(2) + 'x';
  statsElements.position.textContent = 
    `${pos.x.toFixed(1)}, ${pos.y.toFixed(1)}, ${pos.z.toFixed(1)}`;
  statsElements.velocity.textContent = speed.toFixed(1) + ' m/s';
  statsElements.buildings.textContent = buildings.length;
  statsElements.objects.textContent = worldObjects.length;
  
  const now = performance.now();
  if (now - lastFPSUpdate > 1000) {
    fpsValue = frameCount;
    statsElements.fps.textContent = fpsValue;
    frameCount = 0;
    lastFPSUpdate = now;
  }
}
 ...
 // ⚡ OPTIMIZATION: Populate spatial grid with all buildings
 function populateSpatialGrid() {
   ...
   console.log(`⚡ Spatial grid populated with ${buildings.length} buildings`);
 }
   ...
   function updateLOD(time) {
    ...
    cachedObjects.grass.forEach(obj => {
...
function animate(time) {
  ...
  // ⚡ OPTIMIZATION: Update performance monitor
  perfMonitor.update();
  ...
  const rocketInSpace = rocketEnabled && rocketBody && rocketBody.translation().y > ATMOSPHERE_ALTITUDE;
   ...
   if (!rocketInSpace) {
     cachedObjects.lodNodes.forEach(lodNode => {
       if (lodNode) {
  ...
  updateHallway();
   ...
   if (!rocketInSpace) {
     cachedObjects.spinners.forEach(spinner => {
       if (spinner) {
         spinner.rotation.y += (spinner.userData.spinSpeed || 0) * deltaTime;
       }
     });
   }
  ...
  if (time - lastWindowSillUpdate > DOM_UPDATE_INTERVAL) {
    lastWindowSillUpdate = time;
    
    cachedObjects.windowMeshes.forEach(windowMesh => {
      if (windowMesh) {
        windowMesh.visible = optimizationConfig.windows.enabled;
      }
    });
    cachedObjects.sillMeshes.forEach(sillMesh => {
      if (sillMesh) {
        sillMesh.visible = optimizationConfig.sills.enabled;
      }
    });
  }
  ...
  if (droppedViewport && droppedViewportRT && droppedViewportCam) {
    if (time - lastDroppedViewportReadback > DROPPED_VIEWPORT_READBACK_INTERVAL && ensureDroppedViewportBuffers()) {
      lastDroppedViewportReadback = time;
      ...
      renderer.readRenderTargetPixels(
        ...
        0,
        0,
        droppedViewportRT.width,
        droppedViewportRT.height,
        droppedViewportPixels
      );
      
      droppedViewportTempCtx.putImageData(droppedViewportImageData, 0, 0);
      
      droppedViewportHudCtx.save();
      droppedViewportHudCtx.scale(1, -1);
      droppedViewportHudCtx.drawImage(
        droppedViewportTempCanvas,
        0,
        -droppedViewportHudCanvas.height,
        droppedViewportHudCanvas.width,
        droppedViewportHudCanvas.height
      );
      droppedViewportHudCtx.restore();
    }
  }
  ...
  // Update visited buildings for map system
  if (mapOverlay) {
    mapOverlay.updateVisitedBuildings();
  }
  
  // Render map if visible
  if (mapVisible && mapOverlay && time - lastMapUpdate > DOM_UPDATE_INTERVAL) {
    lastMapUpdate = time;
    mapOverlay.renderMap();
  }
...
function setupInteriorUIListeners() {
  ...
  console.log('✓ Interior UI listeners initialized');
}
...
export function boot() {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      ...
      console.log('✓ Interior UI setup complete');
    });
  } else {
    setupInteriorUIListeners();
    console.log('✓ Interior UI setup complete');
